		...
		if ($password !== '') {
			if ($password === $cpassword) {
				if (mb_strlen($_POST['password']) > 7) {
					...
					$_SESSION['msg-s'] = 'Password succeffuly change';
					header('Location: /login.php');
				} else {
					$_SESSION['msg'] = 'Password must be more than 8 characters';
				}
			} else {
				$_SESSION['msg'] = 'Password is not matches';
			}
		} else {
			$_SESSION['msg'] = 'Password mustn`t be empty';
		}
	}
?>
			...
			<div class="main-reg">
				<div class="block-reg">
					...
					<div class="block-reg-container">
						<div class="form-reg">
							<div class="reg">
								<h1 class="h-reg">Change password</h1>
								...
								<form class="form-main" action="" method="POST">
									...
									<input class="reg-submit" type="submit" name="change" value="Change">
								</form>
							</div>
						</div>
					</div>
