	...
	<body>
		<div class="main">
			...
			<dialog class="dialog_tutor">
				<div class="tutor_body">
					<div class="tutor_block">
						...
						<div class="block_text">Ласкаво просимо на арену, хоробрий воїне. Тут ви можете битися з іншими гравцями в битві з використанням богів. При створенні битви або приєднанні до вже існуючої вам надається вибір з цих чотирьох богів різних стихій, а саме Блискавки ⇾ Вогню ⇾ Землі ⇾ Води ⇾ Блискавки, які випадковим чином генерують свою силу.</div>
					</div>
					<div class="tutor_block">
						<div class="block_text">Стрілки також показують перевагу однієї стихії над іншою, яка дає +10% до сили бога. Після того як ви і ваш суперник вибрали бога, переможець визначається автоматично - у кого більше сили за переваги стихій. Під час боїв ви робите ставите срібло, і в разі перемоги отримуєте золото за свою ставку і вашого супротивника мінус 15 %.</div>
			...
			<?if(isset($_POST['window'])){ $battle_id = $_POST['battle_id']; $battle_cost = $_POST['battle_cost'];?>
				<dialog class="dialog">
					<h2 class="dialog_h2">Вступити в бій?</h2>
					<h3 class="dialog_h3">Вартість: <?=$battle_cost?> срібла</h3>
					<form class="dialog_form" action="battles_fight.php" method="POST">
						<input type="hidden" name="battle_id" value="<?=$battle_id?>">
						<input class="dialog_yes" type="submit" name="fight" value="Так">
						<input class="dialog_no" type="submit" name="dia_no" value="Ні" id="closewindow">
					</form>
				</dialog>
			<? } ?>
			<?if ($history_check->num_rows > 0) {?>
				<dialog class="dialog">
					<h2 class="dialog_h2">Ви перемогли!</h2>
					<h3 class="dialog_h3">Ви отримуєте: <?=intval($history['battle_cost']*1.7)?> золота</h3>
			...
			<?} else if ($history_l_check->num_rows > 0 && $history2['enemy_power'] != 0) {?>
				<dialog class="dialog">
					<h2 class="dialog_h2">Ви програли!</h2>
					<h3 class="dialog_h3">Ви втрачаєте: <?=$history2['battle_cost']?> срібла</h3>
					<form class="dialog_form" action="battles.php" method="POST">
			...
			<? } else if ($history_l_check->num_rows > 0 && $history2['enemy_power'] == 0) { ?>
				<dialog class="dialog">
					<h2 class="dialog_h2">Автопрограш!</h2>
					<h3 class="dialog_h3">Ви втрачаєте: <?=$history2['battle_cost']?> срібла</h3>
					<form class="dialog_form" action="battles.php" method="POST">
						<input type="hidden" name="battle_id" value="<?=$history2['battle_id']?>">
						<input class="dialog_yes" type="submit" name="checkBattle" value="OK">
			...
			<div class="battles_main">
				<div class="battles_buttons">
					<a class="battles_button" href="battles_history.php">Історія</a>
					<a class="battles_button" href="battles_create.php">Створити битву</a>
				...
				<div class="battles_block">
					...
					<div class="table_battles">
							<div class="tr_battles">
								<span class="td_battles">Дата створення</span>
								<span class="td_battles">Вартість битви</span>
					...
					?>
							...
							<div class="tr_battles">
								...
								<span class="td_battles"><?=$row4['battle_cost']?> срібла</span>
								<span class="td_battles"><?if ($row4['status'] == 0) { echo 'Очікування противника'; } else { echo 'У процесі'; } ?></span>
								<span class="td_battles"><?=$type . " - " . $row4['god_power'];?> сили</span>
							</div>
							<? } ?>
					<? } else { ?>
						<div class="battles_none">
							У вас немає активних боїв
						</div>
					<? } ?>
					</div>
					<div class="table_header">
					...
					<form class="battles_block_menu" action="" method="POST">
						<label class="label_sort" for="">Сортувати:</label>
						<select onchange="location=value">
							<option value="battles.php">Сортувати</option>
							<option <? if($_GET['sort'] == "100-1000") echo 'selected'?> value="?sort=100-1000">100 - 1000 срібла</option>
							<option <? if($_GET['sort'] == "1000-5000") echo 'selected'?> value="?sort=1000-5000">1000 - 5000 срібла</option>
							<option <? if($_GET['sort'] == "5000-10000") echo 'selected'?> value="?sort=5000-10000">5000 - 10000 срібла</option>
							<option <? if($_GET['sort'] == "10000-50000") echo 'selected'?> value="?sort=10000-50000">10000 - 50000 срібла</option>
							<option <? if($_GET['sort'] == "50000>") echo 'selected'?> value="?sort=50000>">50000 і більше срібла</option>
						</select>
					</form>
					<div class="table_battles">
						<div class="tr_battles">
							<span class="td_battles">Нікнейм</span>
							<span class="td_battles">Дата створення</span>
							<span class="td_battles">Вартість бою</span>
						...
						<div class="tr_battles">
							...
							<span class="td_battles"><?=$row5['battle_cost']?> срібла</span>
							<span class="td_battles">
								<form action="" method="POST">
									...
									<input class="td_battles_button openwindow" type="submit" name="window" value="У бій">
								</form>
							</span>
						</div>
						<?} } else {?>
							<div class="battles_none">
								Немає доступних битв
							</div>
						<? } ?>
					</div>
				</div>
