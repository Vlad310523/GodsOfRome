	...
	<body>
		<div class="main">
			...
			<?if(isset($_POST['window'])){ $amount_gold = $_POST['amount_gold']; if(isset($amount_gold) && $amount_gold > 0){?>
				<dialog class="dialog">
					<h2 class="dialog_h2">Вы уверены?</h2>
					<h3 class="dialog_h3">Вы меняете: <?=$amount_gold?> золота</h3>
					<form class="dialog_form" action="../../inc/swap_gold.php" method="POST">
						<input type="hidden" name="amount" value="<?=$amount_gold?>">
						<input class="dialog_yes" type="submit" name="swap" value="Да">
						<input class="dialog_no" type="submit" value="Нет" id="closewindow">
					</form>
				</dialog>
			<? } else { $_SESSION['msgb'] = 'Количество золота не может быть меньше 1!';} } ?>
			<?if(isset($_POST['withdraw'])){ $withdraw_gold = $_POST['withdraw_gold']; if(isset($withdraw_gold) && $withdraw_gold > 999){?>
				<dialog class="dialog">
					<h2 class="dialog_h2">Вы уверены?</h2>
					<h3 class="dialog_h3">Вы выводите: <?=$withdraw_gold?> золота</h3>
					<form class="dialog_form" action="../../inc/withdraw_gold.php" method="POST">
						<input type="hidden" name="amount" value="<?=$withdraw_gold?>">
						<input class="dialog_yes" type="submit" name="withdraw" value="Да">
						<input class="dialog_no" type="submit" value="Нет" id="closewindow">
					</form>
				</dialog>
			<? } else { $_SESSION['msgb'] = 'Количество золота не может быть меньше 1000!';} } ?>
			<div class="help_main">
				<div class="help_buttons">
					<div class="help_button3">Вывод</div>
				...
				<div class="help_block">
					<div class="block1">
						...
						<form class="buttons_change" action="" method="GET">
							<input class="h1_button blur" type="submit" value="Вывод" name="1">
							<h1 class="h1_button" style="cursor:default;">|</h1>
							<h1 class="h1_button" style="cursor:default;">Обмен</h1>
						</form>
						<div class="swap_block">
							<?if(isset($_SESSION['msgb'])){?><h2 class="msg"><?=$_SESSION['msgb']?></h2><?} unset($_SESSION['msgb']);?>
							<h1 class="h1_block">Обменять золото</h1>
							<h2 class="h2_block">1 золото <img src="../../img/islands/coin_gold.png" alt="coin"> - 1 серебро <img src="../../img/islands/coin_silver.png" alt="coin"></h2>
							<h2 class="h2_block">
								<form class="swap_form" action="" method="POST">
									<label for="swap_text">Количество <img src="../../img/islands/coin_gold.png" alt="gold">:</label>
									<input class="swap_text" type="text" name="amount_gold" id="swap_text">
									<input class="swap_input" name="window" type="submit" value="обмен">
								</form>
							</h2>
						</div>
						<h1 class="h2_block">Последние 20 обменов</h1>
						<div class="table_profile">
							<div class="tr_profile">
								...
								<span class="td_profile" style="border-bottom: 3px solid #70523e;">Серебра</span>
							</div>
						...
						<?} else {?>
						<form class="buttons_change" action="" method="GET">
							<h1 class="h1_button" style="cursor:default;">Вывод</h1>
							<h1 class="h1_button" style="cursor:default;">|</h1>
							<input class="h1_button blur" type="submit" value="Обмен" name="1">
						</form>
						<div class="withdraw_block">
							...
							<h1 class="h1_block">Вывести золото</h1>
							<h2 class="h2_block" style="margin: 15px 0; color: #A60400;">Вы можете выводить только на <a style="margin-left: 5px; text-decoration: underline; color: #A60400;" href="profile.php?1=Withdraw">Payeer кошелек</a>!</h2>
							<h2 class="h2_block">1000 золота <img src="../../img/islands/coin_gold.png" alt="coin"> - 1 доллар $</h2>
							<h2 class="h2_block">
								<form class="swap_form" action="" method="POST">
									<label for="withdraw_text">Количество <img src="../../img/islands/coin_gold.png" alt="gold">:</label>
									<input class="swap_text" type="text" name="withdraw_gold" id="withdraw_text">
									<input class="withdraw_input" name="withdraw" type="submit" value="вывести">
								</form>
							</h2>
						</div>
						<h1 class="h2_block">Последние 20 выводов</h1>
						<div class="table_profile">
							<div class="tr_profile">
								<span class="td_profile" style="border-bottom: 3px solid #70523e;border-right: 3px solid #70523e;">Кошелек</span>
								<span class="td_profile" style="border-bottom: 3px solid #70523e;border-right: 3px solid #70523e;">Дата</span>
								<span class="td_profile" style="border-bottom: 3px solid #70523e;border-right: 3px solid #70523e;">Золота</span>
							...
							<div class="tr_profile">
								...
								<span class="td_profile"><?if ($row4['status'] == 0) { echo 'В процессе'; } else if ($row4['status'] == 1) { echo 'Успешно'; } else { echo 'Отменено'; }?></span>
							</div>
							<? } } else { ?>
							<div class="swap_none">
								У вас нет завершенных выводов
							</div>
							<? } ?>
						</div>
						<?}?>
