	...
	if(isset($_POST['deposit'])){
		...
		if ($_POST['amount'] !== '') {
			if ($amount > 0.99) {
				...
				$_SESSION['msgb'] = 'Количество $ не может быть меньше чем 1.00!';
			}
		} else {
			$_SESSION['msgb'] = 'Количество $ не должно быть пустым!';
		}
	}
?>
				...
				<div class="help_block">
					<div class="block1">
						<div class="swap_block">
							...
							<h1 class="h1_block">Депозит серебра</h1>
							<h2 class="h2_block">1 доллар $ - 1000 серебра <img src="../../img/islands/coin_silver.png" alt="coin"></h2>
							<h2 class="h2_block">
								<label class="method">
							...
							<h2 class="h2_block">
								<form class="swap_form" action="" method="POST">
									<label for="deposit_text">Количество $:</label>
						...
						<h1 class="h2_block">Последние 20 депозитов</h1>
						<div class="table_profile">
							...
							<div class="tr_profile">
								...
								<span class="td_profile"><?if ($row4['status'] == 1) { echo 'Успешно'; } else { echo 'Отменено'; }?></span>
							</div>
							<? } } else { ?>
							<div class="swap_none">
								У вас нет депозитов
