		...
		if ($password !== '') {
			if ($password === $cpassword) {
				if (mb_strlen($_POST['password']) > 7) {
					...
					$_SESSION['msg-s'] = 'Пароль изменен успешно';
					header('Location: login.php');
				} else {
					$_SESSION['msg'] = 'Пароль должен состоять более чем из 8 символов';
				}
			} else {
				$_SESSION['msg'] = 'Пароль не совпадает';
			}
		} else {
			$_SESSION['msg'] = 'Пароль не должен быть пустым';
		}
	}
?>
			...
			<div class="main-reg">
				<div class="block-reg">
					...
					<div class="block-reg-container">
						<div class="form-reg">
							<div class="reg">
								<h1 class="h-reg">Изменить пароль</h1>
								...
								<form class="form-main" action="" method="POST">
									...
									<p class="form-text">Подтвердить пароль</p>	
									<input class="form-input" type="password" name="cpassword" autocomplete="off" maxlength="15">
									<input class="reg-submit" type="submit" name="change" value="Изменить">
								</form>
							</div>
						</div>
					</div>
